...
function draw() {
    ...
    // Dzbanek
    pushMatrix();
    currentColor = [0.440, 0.2, 0.5];
    mat4.translate(modelview, modelview, [1.0, -4.1, -1.5]);
    ...
    for (let i = 0; i < 49; i++) {
        ...
        mat4.rotateZ(modelview, modelview, i * (360 / 49) * (Math.PI / 180));
        mat4.rotateY(modelview, modelview, Math.PI);

        // Pierwszy stożek
        pushMatrix();
        currentColor = [50 / 255, 0.5, 40 / 255];
        mat4.rotateY(modelview, modelview, Math.PI / 2);
        mat4.translate(modelview, modelview, [0, 0, 3.7]);
        mat4.scale(modelview, modelview, [0.7, 0.7, 3]);
        cone.render();
        popMatrix();

        // Drugi stożek - lustrzane odbicie
        pushMatrix();
        currentColor = [50 / 255, 0.5, 40 / 255];
        mat4.rotateY(modelview, modelview, Math.PI / 2);
        mat4.translate(modelview, modelview, [0, 0, -0.7]);
        mat4.scale(modelview, modelview, [0.7, 0.7, 3]); // Odbicie lustrzane
        cone.render();
        popMatrix();
